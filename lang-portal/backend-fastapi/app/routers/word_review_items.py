...
@router.post("/word_review_items/", response_model=schemas.WordReviewItem)
def create_word_review_item(item: schemas.WordReviewItemCreate, db: Session = Depends(get_db)):
    return crud.create_word_review_item(db=db, item=item)

@router.get("/word_review_items/", response_model=list[schemas.WordReviewItem])
...
@router.get("/word_review_items/{item_id}", response_model=schemas.WordReviewItem)
