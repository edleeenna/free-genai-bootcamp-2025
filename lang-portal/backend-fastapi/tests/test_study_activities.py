...
class TestStudyActivities:
    ...
    def test_create_study_activity(self, db_session):
        response = client.post("/study-activities/", json={"name": "Activity 1", "thumbnail_url": "http://example.com/thumbnail1.png", "description": "Description 1"})
    ...
    def test_read_study_activity(self, db_session):
        ...
        response = client.get(f"/study-activities/{study_activity_id}")
        assert response.status_code == 200
    ...
    def test_read_study_sessions(self, db_session):
        ...
        response = client.get(f"/study-activities/{study_activity_id}/study-sessions")
        assert response.status_code == 200
        assert isinstance(response.json(), list)